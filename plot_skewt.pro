; $Id: plot_skewt.pro,v 1.4 2009/08/28 14:48:08 dturner Exp $
;+
; Abstract:
;	This routine builds a skew-T log-P plot.  Each figure requires two
;    calls, one to build the skewT diagram and one to overplot the data.
;    Here is an example:
;
;   IDL> skewt, [-40,30], title='ARM Radiosonde', everyW=2, everySA=2
;   IDL> plot_skewt, temp, rh, pres, col_t=t_color, col_dew=dpt_color, pthick=3
;
;        This routine was found on the web, and I modified it to my purposes.
;
; Input:
...
;
; Keywords:
;	col_t:		The color to make the temperature profile
;	col_dew:	The color to make the dewpoint profile
;	pthick:		The thickness of the lines for the above profile
;
;  		There are other keywords that affect the way the skewT plot
;		looks also -- see the code for more details.
;
; Postscript:
;	Naturally, the figures look the best if they are made in postscript.
;   I have found that this best setup is the following:
;	IDL> device,filename='file.ps',/color,xs=6.5,ys=5.0,/inch,xo=1,yo=1
;
; Final comment:
;       The code uses a "!p.position" to get the plot in the correct place.
;  Thus, at the end of the creation, I need to reset this with the following:
;	IDL> !p.position = 0
;
; Note: I have added another very handy function at the end of this file that
;   creates skew-T plots (with wind barbs) for ARM-formatted radiosondes.  The
;   calling sequence for that script is:
;	
; 	IDL> plot_arm_sonde_skewt, filename
;
;   where "filename" is the path/name of the radiosonde.  This script will 
;   create an encapsulated postscript file in the current directory, and then
;   spawn off a call to convert to make a GIF image.
;-
...
;
;========================================================================
;  Function to compute the saturation mixing ratio given T & P
...
;  on top of a skew-T, Log(P) diagram.  Note that the moisture profile
;  needs to be an RH profile in %RH, and a dewpoint profile will be computed.
;  Temperature must be in C.
PRO plot_skewt, t, rh, p, col_t=col_t,col_dewpt=col_dewpt, pthick=pthick

COMMON RANGES, trange, prange

;  Set the keywords
if(n_elements(pthick) eq 0) then pthick = 3

;  Compute the dew point profile
td = rh2dpt(t,rh/100.)
...
;  Plot temperature sounding data.
   ...
   IF n_elements(col_t) EQ 1 THEN $
	plots, [x0, x1], [y0, y1], clip=clip, noclip=0, thick=pthick ,color=col_t $
   ELSE plots, [x0, x1], [y0, y1], clip=clip, noclip=0, thick=pthick
...
;  Plot dew point temperature sounding data.
   ...
   IF n_elements(col_dewpt) EQ 1 THEN $
   	plots, [x0, x1], [y0, y1], clip=clip, noclip=0, thick=pthick, color=col_dewpt $
   ELSE plots, [x0, x1], [y0, y1], clip=clip, noclip=0, thick=pthick
endfor
...
;==========================================================================
; This routine makes a nice plot of an ARM radiosonde file
pro plot_arm_sonde_skewt, filename
  ...
  ncdf_varget,fid,'pres',p
  ncdf_varget,fid,'tdry',t
  ncdf_varget,fid,'rh',rh
  ncdf_varget,fid,'wspd',wspd
  ncdf_varget,fid,'deg',wdir
  ncdf_close,fid

  systime2ymdhms,bt,yy,mm,dd,hh,nn,ss
  mnth = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
  dates = string(format='(I0,1x,A,1x,I0,A,I2.2,A,I2.2,A)',dd(0),mnth(mm(0)-1),yy(0), $
  	' at ',hh(0),':',nn(0),' UTC')

  idx = 0
  npts = 20
  dpres = 10^[findgen(npts+1)/float(npts)*(alog10(1050)-alog10(100))+alog10(100)]
  for i=0,n_elements(dpres)-1 do begin
    foo = where(p lt dpres(i) and wdir gt -400 and wspd gt -1, nfoo)
    if(nfoo gt 0) then idx = [idx,foo(0)]
  endfor

	; Get the magnitudes (m/s)
  wdir = wdir(idx)
  wspd = wspd(idx)

  parts = str_sep(files(0),'/')
  sname = parts(n_elements(parts)-1)
  parts = str_sep(sname,'.')
  sname = parts(0)
  for i=1,n_elements(parts)-2 do sname = sname + '.' + parts(i)
  epsname = sname + '.eps'
  gifname = sname + '.gif'
  ...
  set_plot,'ps'
  !p.multi=[0,1,2]
  device,filename=epsname,/color,/encap,xs=8.5,ys=5.0,/inch

  skewt,[-40,30],title=dates,everyW=2,everySA=2

  @color_syms.include

  plot_skewt,t,rh,p,col_t=d_black,col_dew=d_blue, pthick=6
  !p.position=0

  !p.region=[0.65,0.10,0.99,0.9]
  x = replicate(0,n_elements(idx))
  y = p(idx)
  	; The "wind_barb" program wants to display the data in knots. We want
	; the data in m/s, so I need to multiply by this factor so that the
	; wind_barb code can undo it...
  kn2mps = 0.5147965      ; multiply knots by this factor to get m/sec
  	
  plot,[-1,1]*0.6,[3.02,2.00],chars=0.01,xst=5,yst=5,yr=[3.02,2.0],/nodata
  for i=0,n_elements(wspd)-1 do $
  	wind_barb,wspd(i)*kn2mps,180-wdir(i),x(i),alog10(y(i)),size=0.1

  device,/close
  !p.region=0
  !p.multi=0
  spawn,'convert -density 100x100 '+epsname+ ' ' + gifname
