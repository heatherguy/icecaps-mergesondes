; $Id: ps_form.pro,v 1.2 1997/10/05 17:51:25 d3h797 Release_ddt_1_13 $
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;+
;Abstract:
; NAME:
...
;
; PURPOSE:
;   This function puts up a form the user can manipulate to configure a
;   PostScript device driver. The function result (ONLY if the user
;   selects the ACCEPT button) can be sent directly to the DEVICE procedure
;   by means of its _Extra keyword.
;
; MAJOR TOPICS:
...
;    If this keyword is not used, the procedure Set_Personal_Local_Defaults
...
; USAGE:
;
;  The calling procedure for this function will look something like this:
;
;     formInfo = PS_FORM(Cancel=cancelButton)
;
...
;
;   None. Designed to work originally in conjunction with GWindow,
...
;      25 Nov 96.
;   Fixed a bug in how YOFFSET was calculated when in Landscape mode. 27
;Nov 96.
;

Pro Set_Personal_Local_Defaults, ptr
...
personalDefaults =  { PS_FORM_INFO, $
                    ...
                    filename:'davidf.ps', $  ; The name of the output file
                    inches:1, $              ; Inches on.
                    color:0, $               ; Color off.
...
Function PlotBox_Coords, xsize, ysize, xoff, yoff, inches
...
IF landscape EQ 1 THEN BEGIN
   ...
   coords = PlotBox_Coords(xsize, ysize, xoff, yoff, inches)
...
Pro PSFormMoveBox, event

   ; This is the event handler that allows the user to "move"
   ; the plot box around in the page window. It will set the
   ; event handler back to "PSFormDrawEvents" when it senses an
   ; "UP" draw button event and it will also turn Draw_Motion_Events
   ; OFF.
...
Pro PsFormDrawBox, event
   ...
   ; back to PSFormDrawEvents and turn Draw_Motion_Events to OFF.
...
whatButtonPressed = What_Button_Pressed(event)
CASE whatButtonPressed OF

   'RIGHT': Begin

         ; Resize the plot box interactively. Change the event handler
         ; to PSFormDrawBox. All subsequent events will be handled by
         ; PSFormDrawBox until an UP event is detected. Then you will
         ; return to this event handler. Also, turn motion events ON.

      Widget_Control, event.id, Event_Pro='PSFormDrawBox', $
   ...
   'LEFT': Begin
         ...
         ; Resize the plot box interactively. Change the event handler
         ; to PSFormMoveBox. All subsequent events will be handled by
         ; PSFormMoveBox until an UP event is detected. Then you will
         ; return to this  event handler. Also, turn motion events ON.
     ...
     ENDIF
      ...
      Widget_Control, event.id, Event_Pro='PSFormMoveBox', $
         Draw_Motion_Events=1
...
CASE thisButton OF
   ...
   'LANDSCAPE': Begin
     ...
     ; Calculate new offsets.
      ...
      box = PlotBox_Coords(xsize, ysize, xoff, yoff, info.inches)
   ...
   'ACCEPT': Begin
      ...
      IF info.landscape EQ 1 THEN Begin
          ...
          yoff = pagesize - xoff    ; Offset really calculated from right
;of page
          xoff = temp
...
IF N_Elements(localDefaults) EQ 0 THEN BEGIN
   ...
   Set_Personal_Local_Defaults, localDefaultPtr
...
sizebase = Widget_Base(tlb, Row=1, Frame=1)
   ...
   numbase = Widget_Base(sizebase, Column=1)
      ...
      numsub2 = Widget_Base(numbase, Row=1, Event_Pro='PSFormNumEvents')
...
drawbase = Widget_Base(sizebase, Row=1)
   ...
   draw = Widget_Draw(drawbase, XSize=114, YSize=114, $
     Event_Pro='PSFormDrawEvents', Button_Events=1)

colorbase = Widget_Base(tlb, Row=1, Frame=1)
   ...
   bitsw = Widget_Base(colorbase, Row=1, /Exclusive, Frame=1)
...
orientbase = Widget_Base(tlb, Row=1, Frame=1)
...
filenamebase = Widget_Base(tlb, Row=1, Frame=1)

   textlabel = Widget_Label(filenamebase, Value='Filename: ')

       ; Set up text widget with an event handler that ignores any event.

   filenamew = Widget_Text(filenamebase, /Editable, Scr_XSize=250,  $
      Value=filename, Event_Pro='Ignore_Event')
...
buttonbase = Widget_Base(tlb, Row=1, Frame=1)
   ...
   sysdefaults = Widget_Button(buttonbase, Value='System Defaults', $
      Event_Pro='Set_System_Defaults', UValue='SYSTEM')
   localdefs = Widget_Button(buttonbase, Value='Local Defaults', $
      Event_Pro='Set_Local_Defaults', UValue='LOCAL')
...
boxInfo = PlotBox_Coords(xsize, ysize, xoff, yoff, inches)
...
IF landscape EQ 0 THEN PlotS, portraitbox_x, portraitbox_y, /Device,$
Color=ncol-2  ELSE $
...
info = {draw:draw, $          ; The draw widget id
      ...
      port:port, $            ; The widget id of the portrait button.
    	...
    	pplotbox_x:[8, 8, 82, 82, 8], $       ; Portrait box outline x
;(default)
    	pplotbox_y:[60, 106, 106, 60, 60], $  ; Portrait box outline y
;(default)
...
ENDIF ELSE formInfo = 0 ; Return a throw-away value
   ...
   ; Free up the space allocated to the pointer and the data
