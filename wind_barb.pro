...
FOR i=0,npennant-1 DO BEGIN
    ...
    penn[*,0]=[0,0.]
    penn[*,1]=[0.4,0.3]
    penn[*,2]=[0,0.2]	; Original

    		; this was needed to get the barbs on the plot_skewt.pro
		; script to plot in the right direction, which was required
		; because the y-axis was decreasing with altitude (pressure)
    penn[*,1]=[-0.4,0.3]
    ; rotate
    orig=MROT#orig
    penn=MROT#penn
