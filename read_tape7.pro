...
;		level_water_vapor_mixing_ratio=level_water_vapor_mixing_ratio
;
; INPUTS:
...
;
; $Id: read_tape7.pro,v 1.6 2006/12/12 12:35:08 dturner Exp $
;
;  --------------------- Paul vanDelst's RCS information -------------------------
; Id: read_tape7.pro,v 1.2 1998/11/16 18:50:18 paulv Exp $
; Log: read_tape7.pro,v $
; Revision 1.2  1998/11/16 18:50:18  paulv
; Added keyword to allow return of the water vapour mixing ratio.
;
; Revision 1.1  1997/03/29 16:56:11  paulv
; Initial revision
;
; Revision: 1.2 $
; Source: /home4/cvsroot/paulv/idl/LBLRTM/read_tape7.pro,v $
;
;-
...
;  assertion, ( N_PARAMS() NE 2 ), $
;             'Must specify TAPE7 filename and return structure variable'
...
; -----------------------------------------------
; Interpolate this to the level boundaries [g/kg]
; -----------------------------------------------
 
  if(n_elements(atm_data) gt 0) then begin
    layer_pressure = atm_data.pressure
    level_pressure = [reform(atm_data.p_bot),atm_data.p_top(atm_data.n_layers-1)]
  endif else begin
    layer_pressure = tape7_data.pressure
    level_pressure = [reform(tape7_data.p_bot),tape7_data.p_top(tape7_data.n_layers-1)]
  endelse
  level_water_vapor_mixing_ratio = interpol(water_vapor_mixing_ratio, $
  		layer_pressure, level_pressure)
